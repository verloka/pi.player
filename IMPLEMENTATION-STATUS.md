...
- The backend sets device volume and reads temperature by running `wpctl` and `vcgencmd` directly (the kernel thermal zone is the fallback). wpctl needs the desktop session's PipeWire, so `configure-kiosk.sh` runs the service as the desktop user with `NoNewPrivileges` kept on. Parsing and command lines are covered by backend tests with a simulated device; real `wpctl` and `vcgencmd` on a Pi are **not verified in hardware**.
